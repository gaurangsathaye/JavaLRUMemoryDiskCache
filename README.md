...
* `public T get(String key)` - Gets your object from the memory or disk cache.  If object is not in cache, it is loaded and put into the cache. You tell the cache how to load your objects (see below).  There are other methods like `getOnly` and `putOnly` but this is probably the only method you need.
